...
import net.muellersites.advancedsteelmaking.api.Enums;
...
{
    public interface IASMetaBlock {
        String getASBlockName();
    ...
    public interface IUsesBooleanProperty {
        ASProperties.PropertyBoolInverted getBoolProperty(Class<? extends IUsesBooleanProperty> inf);
    }
    ...
    public interface IDirectionalTile {
        ...
        default EnumFacing getFacingForPlacement(EntityLivingBase placer, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
            ...
            LogHelper.info("Getting Facing for Placement; limit: " + limit);
            if(limit==0)
                f = side;
            else if(limit==1)
    ...
    public interface IConfigurableSides {
        Enums.SideConfig getSideConfig(int side);
